...
@interface ShredderBackSceneMesh() {
...
@implementation ShredderBackSceneMesh
...
{
    int totalColumnCount = xResolution;
    xResolution -= 1;
    ...
    _centerLocation = 0;
    vertexCount = (GLsizei)((xResolution) * (yResolution + 1));
    ...
    for (int y = 0; y < yResolution + 1; y++) {
        ...
        for (int x = 0; x < xResolution; x++) {
            GLfloat tx = (x + 1)/ (GLfloat)totalColumnCount;
            ShredderVertex *vertex = &vertices[y * (xResolution) + x];
    ...
    indexCount = xResolution * yResolution * 3;
    GLushort *indicies = malloc(indexCount * sizeof(GLushort));
    for (int y = 0; y < yResolution; y++) {
        for (int x = 0; x < xResolution / 2; x++) {
            int idx = (y * xResolution / 2) + x;
            int i = y * xResolution + x * 2;
            indicies[idx * 6 + 0] = i;
            indicies[idx * 6 + 1] = i + 1;
            indicies[idx * 6 + 2] = i + xResolution;
            indicies[idx * 6 + 3] = i + xResolution;
            indicies[idx * 6 + 4] = i + 1;
            indicies[idx * 6 + 5] = i + xResolution + 1;
        }
    }
...
- (void) drawEntireMesh
{
    for (int i = 0; i < self.xResolution / 2 ; i++) {
